...
1. **Measurement**: We use `OffscreenMeasurer.tsx` (from `@formcast/react`) to get the height of every node.
...
Here is the core recursive splitting logic from `@formcast/engine/src/PaginationEngine.ts`:
...
To solve this, the core `PaginationEngine` in `@formcast/engine` relies on the `IMeasurer` adapter, which is implemented by `@formcast/react`'s `OffscreenMeasurer.tsx`.
