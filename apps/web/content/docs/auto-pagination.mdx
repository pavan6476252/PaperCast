...
If you toggle `pageBreakBefore: true`, PaperCast will force a page break immediately _before_ this element renders.
...
- **Use Case**: You have a heading (e.g., "Summary"), but there isn't enough room on the page for the paragraph below it. Instead of leaving the heading orphaned at the bottom of page 1, PaperCast will push _both_ the heading and the paragraph to page 2.

### 3. Break Inside Avoid

By default, PaperCast will try to split large containers (like a multi-line paragraph or a table) perfectly in half when it crosses a page boundary. If you set `breakInside: avoid`, PaperCast will refuse to split the element. If it doesn't fit on the current page, the _entire_ element is pushed to the next page.
