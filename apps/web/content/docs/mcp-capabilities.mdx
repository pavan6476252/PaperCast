...
The FormCast MCP Server provides five core tool modules. Agents use these tools to manipulate the document state, orchestrate pagination, and manage browser sessions without modifying the underlying JSON AST directly.
...
- **`list_formcast_skills`**: Returns a list of available AI skills (markdown files containing domain rules).
- **`read_formcast_skill`**: Reads the content of a specific skill file to teach the agent how to perform a task.
