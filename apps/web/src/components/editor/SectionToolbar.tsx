...
import { PageRegion } from "@papercast/core";
...
export const SectionToolbar: React.FC<SectionToolbarProps> = ({
  ...
  const currentConditionType =
    typeof section.condition === "object"
      ? section.condition.type
      : section.condition || "all";

  if (parsedDocument && currentConditionType !== "custom") {
    const sections =
      type === "header"
        ? parsedDocument.document.headers
        : parsedDocument.document.footers;
    const conflictingId = Object.keys(sections || {}).find((sid) => {
      const s = sections[sid];
      const sType =
        typeof s.condition === "object"
          ? s.condition.type
          : s.condition || "all";
      return sid !== id && sType === currentConditionType;
    });
    if (conflictingId) {
      conditionError = `⚠️ The '${currentConditionType}' condition is already used by '${sections[conflictingId].name || conflictingId}'. Only one ${type} should use this condition.`;
    }
  }
