  ...
  const sidebarContent = (
    <>
      <div className="flex items-center gap-2 mb-8">
        ...
        <h2 className="text-xl font-bold text-foreground">Schema Library</h2>
      </div>
            ...
            return (
              <li key={category.id} className="relative group">
                ...
                >
                  ...
                  <span
                    className={`text-[10px] font-bold px-1.5 py-0.5 rounded-full ${isActive ? "bg-background text-foreground shadow-sm" : "bg-surface text-foreground/60 group-hover:bg-background"}`}
                  >
                    {count}
                  </span>
                </button>
      ...
      <main className="flex-1 flex flex-col p-4 md:p-8 overflow-hidden bg-background">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8 pr-2">
          ...
          <div className="relative w-full sm:w-72 shrink-0">
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 text-sm border border-border rounded-xl focus:outline-none focus:ring-2 focus:ring-accent focus:border-transparent bg-background text-foreground placeholder:text-foreground/40 shadow-sm"
            />
        ...
        <div className="w-full flex-1 overflow-y-auto pr-2 pb-8">
          {filteredSnippets.length > 0 ? (
            ...
            <div className="flex-1 flex flex-col items-center justify-center h-64 text-foreground/60 bg-background rounded-2xl border border-border shadow-sm border-dashed">
              <Search className="w-12 h-12 mb-4 text-foreground/40" />
              ...
              <button
                ...
                className="mt-4 px-4 py-2 bg-surface hover:bg-surface/80 text-foreground text-sm font-medium rounded-lg transition-colors"
              >
                Clear Search
              </button>
            </div>
