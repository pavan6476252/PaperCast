...
export function registerSkillsTools(server: McpServer) {
  ...
  server.registerTool(
    "read_papercast_skill",
    {
      description:
        "Read a specific PaperCast skill or documentation topic. You must pass the exact filename (e.g. 'schema-BoxModel.md' or 'widget-TableNode.md') as returned by 'list_papercast_skills'.",
      inputSchema: {
        skillSlug: z
          .string()
          .describe(
