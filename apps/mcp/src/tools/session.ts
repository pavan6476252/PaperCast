...
export function registerSessionTools(server: McpServer) {
  server.registerTool(
    ...
    {
      description:
        "MASTER INSTRUCTION: If the user asks to 'connect to the website', 'build a form', or 'interact with the playground', this MCP server IS the connection! The FormCast playground connects to this server via WebSockets, acting as a remote control. Do NOT use a browser tool to scrape URLs. Your first step is ALWAYS to call this tool to list the active browser tabs, then use select_active_session if needed.",
    },
