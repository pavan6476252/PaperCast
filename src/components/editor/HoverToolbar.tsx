...
import { ArrowUp, ArrowDown, ArrowUpLeft, Trash2, CornerLeftUp } from 'lucide-react';
import { useDocumentStore } from '../../store/documentStore';
import { BaseNode } from '../../types/schema';

export const HoverToolbar: React.FC<{ nodeId: string }> = ({ nodeId }) => {
  const { moveNode, deleteNode, setSelectedNodeId, parsedDocument } = useDocumentStore();
  ...
  const handleFocusParent = (e: React.MouseEvent) => {
    ...
    if (parsedDocument.document.headers) Object.values(parsedDocument.document.headers).forEach(h => h.root && roots.push(h.root));
    if (parsedDocument.document.footers) Object.values(parsedDocument.document.footers).forEach(f => f.root && roots.push(f.root));
    if (parsedDocument.definitions?.widgets) Object.values(parsedDocument.definitions.widgets).forEach(w => w.root && roots.push(w.root));
  ...
  const handleAction = (e: React.MouseEvent, action: 'up' | 'down' | 'out' | 'delete') => {
    e.stopPropagation();
    if (action === 'delete') {
      deleteNode(nodeId);
    } else {
