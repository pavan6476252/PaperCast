...
export default function LandingPage() {
  return (
    <div className="relative flex flex-col items-center overflow-x-hidden">
      ...
      <div className="absolute top-0 -right-10 w-96 h-96 bg-blue-300 rounded-full mix-blend-multiply filter blur-[100px] opacity-40 animate-blob" style={{ animationDelay: "2s" }}></div>
      <div className="absolute top-40 left-1/2 -translate-x-1/2 w-96 h-96 bg-pink-200 rounded-full mix-blend-multiply filter blur-[100px] opacity-30 animate-blob" style={{ animationDelay: "4s" }}></div>

      {/* Hero Section */}
      <div className="relative z-10 w-full max-w-5xl px-4 pt-32 pb-20 text-center flex flex-col items-center">
        <div className="inline-flex items-center rounded-full border border-blue-200 bg-blue-50/50 backdrop-blur-sm px-4 py-1.5 text-sm font-medium text-blue-700 mb-8 opacity-0 animate-fade-in-up" style={{ animationDelay: "0.1s" }}>
          <span className="flex h-2 w-2 rounded-full bg-blue-600 mr-2 animate-pulse"></span>
          Now available in early access
        </div>
        
        <h1 className="relative text-5xl font-extrabold tracking-tight sm:text-7xl lg:text-[5.5rem] leading-[1.1] text-slate-900 mb-8 opacity-0 animate-fade-in-up" style={{ animationDelay: "0.2s" }}>
        ...
        <p className="max-w-2xl text-lg text-slate-600 sm:text-xl leading-relaxed opacity-0 animate-fade-in-up" style={{ animationDelay: "0.3s" }}>
          Design, validate, and render complex documents and PDFs dynamically. 
          Use our visual playground to build templates backed by strong JSON schemas.
        </p>
        
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4 pt-10 opacity-0 animate-fade-in-up" style={{ animationDelay: "0.4s" }}>
          <Link
            href="/playground"
      ...
      {/* Grid of features */}
      <div className="w-full max-w-6xl mx-auto px-4 py-24 opacity-0 animate-fade-in-up" style={{ animationDelay: "0.6s" }}>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {[
            ...
            <div key={i} className="group relative rounded-3xl p-[1px] shadow-sm hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-500 hover:-translate-y-1 overflow-hidden bg-white/60">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-12 md:gap-8 mb-12">
            <div className="md:col-span-5">
              <Link href="/" className="flex items-center space-x-2 mb-6 group">
                <div className="w-10 h-10 bg-blue-600 rounded-xl flex items-center justify-center text-white font-bold text-2xl shadow-lg shadow-blue-600/30 group-hover:scale-105 transition-transform">F</div>
                <span className="inline-block font-extrabold text-2xl tracking-tight text-slate-900">FormCast</span>
              </Link>
              <p className="text-slate-500 text-base leading-relaxed max-w-sm mb-8">
                The modern, schema-first document builder for creating pixel-perfect, auto-paginated PDFs and reports with ease.
              </p>
              <div className="flex gap-4">
                {/* Social icons placeholders */}
                {['Twitter', 'GitHub', 'Discord'].map((social, i) => (
                  <a key={i} href="#" className="w-10 h-10 rounded-full bg-slate-50 border border-slate-200 flex items-center justify-center text-slate-500 hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition-all hover:-translate-y-1">
            ...
            <div className="md:col-span-2 md:col-start-8">
              ...
              <ul className="space-y-3.5">
                {['Playground', 'Widget Registry', 'Documentation', 'Templates'].map((link, i) => (
                  <li key={i}>
                    <a href="#" className="text-sm text-slate-500 hover:text-blue-600 hover:translate-x-1 transition-all inline-block">{link}</a>
                  </li>
                ))}
              </ul>
            </div>
