...
import { row, column, text, spacer, image, table } from "@/store/test.data";
...
export const advanced: WidgetRegistryItem[] = [
  ...
  },
  {
    ...
    description: "A split row containing both tabular data and a summary text column.",
    schema: wrap(
      row([
        table(
          [
            { headerText: "Item", bindPath: "items.desc", flex: 2 },
            { headerText: "Cost", bindPath: "items.total", flex: 1, align: "right" }
          ],
          "items"
        ),
        spacer(24),
        {
          ...column([
            ...
            spacer(12),
            text("This month's expenses have increased by 15% due to additional server hosting and consulting fees.", { lineHeight: 1.5, color: "#444" })
          ], { width: 250, paddingLeft: 16, paddingRight: 16, paddingTop: 16, paddingBottom: 16 }),
