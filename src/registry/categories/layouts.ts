...
import { row, column, text, spacer, divider, addressBlock, summaryCard } from "@/store/test.data";
...
export const layouts: WidgetRegistryItem[] = [
  {
    ...
    schema: wrap(
      row([
        addressBlock("BILL TO", ["customer.name", "customer.address", "customer.email"]),
        addressBlock("SHIP TO", ["customer.name", "customer.address", "customer.email"])
