...
Every node requires a universally unique `id`. When a widget is dropped from the registry, FormCast automatically generates a new UUID for that node to ensure the AST remains strictly valid.
</Callout>
