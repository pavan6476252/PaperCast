...
Notice the `totalPages` argument. Because we don't know the total number of pages until pagination finishes, FormCast performs a **Two-Pass** pagination. It paginates once to find `totalPages`, then paginates a second time to inject the correct `"last"` page headers and footers.
</Callout>
